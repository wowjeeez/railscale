...
impl ForwardHttpToHttps {
    pub async fn new(bind: &str, upstream: &str, tls_config: Arc<rustls::ClientConfig>) -> Result<Self, RailscaleError> {
    ...
    pub fn builder() -> ForwardHttpToHttpsBuilder {
        ForwardHttpToHttpsBuilder {
            bind_addr: None,
            upstream: None,
            tls_config: None,
            buffer_limits: BufferLimits::default(),
...
pub struct ForwardHttpToHttpsBuilder {
...
impl ForwardHttpToHttpsBuilder {
    ...
    pub async fn build(self) -> Result<ForwardHttpToHttps, RailscaleError> {
        ...
        let tls_config = self.tls_config.ok_or_else(|| {
            RailscaleError::from(ErrorKind::RoutingFailed("tls_config required".into()))
        })?;
        let source = TcpSource::bind(&bind).await?;
        Ok(ForwardHttpToHttps {
            source,
            upstream,
            tls_config,
            buffer_limits: self.buffer_limits,
